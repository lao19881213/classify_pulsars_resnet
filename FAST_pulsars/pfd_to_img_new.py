    ...
    def getdata(self, phasebins=0, freqbins=0, timebins=0, DMbins=0, intervals=0, subbands=0, bandpass=0, ratings=None):
        ...
        def getintervals(M):
            ...
            if not feature in self.extracted_feature:
                ...
                ax.set_axis_off()
                #fig.add_axes(ax)
                plt.imsave("%s_intervals.png" % os.path.join(output_dir, fn), img, origin='lower', cmap=plt.cm.gray_r) 
        ...
        def getsubbands(M):
            ...
            if not feature in self.extracted_feature:
                ...
                ax.set_axis_off()
                #fig.add_axes(ax)
                plt.imsave("%s_subbands.png" % os.path.join(output_dir, fn), img, origin='lower', cmap=plt.cm.gray_r)
